...
import com.yimi.rentme.utils.water.helper.MagicFilterFactory;
import com.yimi.rentme.utils.water.helper.MagicFilterType;
import com.yimi.rentme.utils.water.surface.InputSurface;
import com.yimi.rentme.utils.water.surface.OutputSurface;
...
//                muxAudioTrack = mMediaMuxer.addTrack(format);
    ...
    private void initVideoCodec() {
        ...
        info.setRotation(videoRotation);

        MediaFormat mediaFormat;
        if (info.getRotation() == 0 || info.getRotation() == 180) {
            mediaFormat = MediaFormat.createVideoFormat("video/avc", info.getWidth(), info.getHeight());
        }else {
            mediaFormat = MediaFormat.createVideoFormat("video/avc", info.getHeight(), info.getWidth());
        }
        //设置视频的编码参数
