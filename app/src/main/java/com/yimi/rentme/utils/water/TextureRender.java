...
import com.yimi.rentme.utils.water.filter.AFilter;
import com.yimi.rentme.utils.water.filter.GroupFilter;
import com.yimi.rentme.utils.water.filter.NoFilter;
import com.yimi.rentme.utils.water.filter.RotationOESFilter;
import com.yimi.rentme.utils.water.filter.WaterMarkFilter;
import com.yimi.rentme.utils.water.util.EasyGlUtils;
import com.yimi.rentme.utils.water.util.MatrixUtils;
...
//        st.getTransformMatrix(mSTMatrix);//当视频角度不为0时,这里会导致视频方向不对
    ...
    public void surfaceCreated() {
        ...
        if (info.getRotation() == 0 || info.getRotation() == 180) {
            EasyGlUtils.genTexturesWithParameter(2, fTexture, 0, GLES20.GL_RGBA, info.getWidth(), info.getHeight());
            viewWidth = info.getWidth();
            viewHeight = info.getHeight();
        } else {
            EasyGlUtils.genTexturesWithParameter(2, fTexture, 0, GLES20.GL_RGBA, info.getHeight(), info.getWidth());
            viewWidth = info.getHeight();
            viewHeight = info.getWidth();
        }

        rotationFilter.setRotation(info.getRotation());
    }
    ...
    public void addGpuFilter(GPUImageFilter filter) {
        ...
        if (filter != null) {
            mGpuFilter.init();
            mGpuFilter.onDisplaySizeChanged(info.getWidth(), info.getHeight());
            mGpuFilter.onInputSizeChanged(info.getWidth(), info.getHeight());
        }
    }
    ...
    public void setVideoWidthAndHeight(VideoInfo info) {
        rotationFilter.setRotation(info.getRotation());
        if (info.getRotation() == 0 || info.getRotation() == 180) {
            this.videoWidth = info.getWidth();
            this.videoHeight = info.getHeight();
        } else {
            videoWidth = info.getHeight();
            videoHeight = info.getWidth();
        }
    }
